...
/**
* @file nggmL1b2SatelliteTracking.cpp
*
* @brief Read NGGM L1B data.
*
...
from the NGGM SDS format (KBR1B or LRI1B) into \file{instrument file (SATELLITETRACKING)}{instrument}.
...
If only A-channel SNR values are available, K\_B\_SNR and Ka\_B\_SNR are written as 0
to keep the output layout consistent with \program{GraceL1b2SatelliteTracking}.
...
class NggmL1b2SatelliteTracking
{
public:
  void run(Config &config, Parallel::CommunicatorPtr comm);
};

GROOPS_REGISTER_PROGRAM(NggmL1b2SatelliteTracking, SINGLEPROCESS, "read NGGM L1B data (KBR1B or LRI1B)", Conversion, Grace, Instrument)

/***********************************************/

void NggmL1b2SatelliteTracking::run(Config &config, Parallel::CommunicatorPtr /*comm*/)
{
  try
  {
    ...
    for(UInt idFile=0; idFile<fileNameIn.size(); idFile++)
    {
      ...
      for(UInt idEpoch=0; idEpoch<numberOfRecords; idEpoch++)
      {
        ...
        UInt16   K_A_SNR, Ka_A_SNR, K_B_SNR = 0, Ka_B_SNR = 0;
        Byte     qualflg;
        ...
        try
        {
          ...
          file>>K_A_SNR>>Ka_A_SNR;
          file>>FileInGrace::flag(qualflg);
        }
        catch(std::exception &/*e*/)
        {
          // NGGM number of records issue
